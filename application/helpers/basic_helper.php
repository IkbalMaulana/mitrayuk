...
if ( ! function_exists('generateCode'))
{
	...
	{						
		...
		if ($par == 'surat_pengiriman') {
			$string .= "/AD.SP";
		}elseif ($par == 'tanda_terima') {
			$string .= "/AD.TT";
		}elseif ($par == 'faktur') {
			$string .= "/AD.FP/".romanic_number(date('m')).'/'.date('Y');
		}elseif ($par == 'kwitansi') {
			$string .= "/AD.KWT/".romanic_number(date('m')).'/'.date('Y');
		}
...
function hak_akses($type="")
{
	...
	$akses = $CI->model->join('user','*',array(array('table'=>'privilege_user','parameter'=>'user.roleUser=privilege_user.idRole'),array('table'=>'master_menu','parameter'=>'privilege_user.menuPrivilege=master_menu.idMenu')),"idRole='".$CI->session->userdata('roleUser')."' and actionPrivilege='".$actionPrivilege."' and targetMenu='".strtolower(getModule().'/'.getController())."'");
...
function show_errPrivilege()
{
	$CI = &get_instance();
	$data['content'] = 'backend/err_privilege';
	$CI->load->view('backend/main', $data, FALSE);
